...
object ParboiledParser {
  ...
  case class Rule(name: Identifier, body: Expr) extends Expr
  ...
  def pretty(expr: Expr): String = expr match {
    ...
    case Rule(name, body) => "def " + pretty(name) + " = " + pretty(body)
...
class ParboiledParser(val input: ParserInput) extends Parser {
  ...
  def PbRule = rule { "def" ~ Ident ~ "=" ~ Body ~> Rule }
  ...
  def SimpleExpr: Rule1[Expr] = rule { TestRule | NegateRule | OneOrMoreRule | ZeroOrMoreRule | 
                                       OptionalRule | StringLiteralRule | Ident | ('(' ~ Body ~ ')') }
  ...
  def StringLiteralRule = rule { WhiteSpace ~ '"' ~ capture(zeroOrMore(AlphaNum)) ~ '"' ~ WhiteSpace ~> StringLiteral }
